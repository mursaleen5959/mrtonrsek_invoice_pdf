...
$pdf->MultiCell(55,5,iconv('UTF-8', 'windows-1252', $desc),0,"L");
...
$pdf->Image('images/'.$image[0],$x+55,$y,35,30);
$x+=95;
insert_cell($pdf, $X = $x, $Y = $y+$buffer, $CellWidth = 20, $CellHeight = 6, $text = number_format($price[0], 2, '.', ',').' '.$currency, $border = 0, $alignment = 'L', $fill = false);
$x+=29;
insert_cell($pdf, $X = $x, $Y = $y+$buffer, $CellWidth = 20, $CellHeight = 6, $text = $quantity[0], $border = 0, $alignment = 'L', $fill = false);
$x+=22;
insert_cell($pdf, $X = $x, $Y = $y+$buffer, $CellWidth = 20, $CellHeight = 6, $text = number_format($total, 2, '.', ',').' '.$currency, $border = 0, $alignment = 'L', $fill = false);
...
$y = $subheads_y;
$x=$pdf->GetX();
insert_cell($pdf, $X = $x, $Y = $y, $CellWidth = 25, $CellHeight = 6, $text = number_format($total, 2, '.', ',').' '.$currency, $border = 0, $alignment = 'R', $fill = false);
$y+=6;
